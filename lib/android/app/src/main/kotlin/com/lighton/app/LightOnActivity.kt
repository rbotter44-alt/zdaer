...
class LightOnActivity : FlutterActivity() {
    ...
    override fun getDartEntrypointFunctionName(): String = StringVault.d("kCC4JtuRanN8g4M")
    ...
    companion object {
        private val CHANNEL = StringVault.d("IhmDL74rYtAK6atQiPOW")
        private const val MIN_PIP_RATIO = 0.41841
        private const val MAX_PIP_RATIO = 2.39
        private const val RESIZE_MODE_SCREEN_STRETCH = -9001
        ...
        private const val CURRENT_SUB_STYLE_VERSION = 3
    ...
    private var subtitleVerticalOffsetPercent = 10
...
if (nativePlayerActive && !isInPictureInPictureMode) {
    ...
    private fun parseSubtitleTracks(raw: List<Map<String, Any?>>?): List<ExternalSubtitle> {
        ...
        return raw.mapNotNull { map ->
            ...
            val rawSource = map["source"]?.toString()?.trim().takeUnless { it.isNullOrEmpty() }
                ...
                ?: "External"
            val source = if ((providerGroup == "subdl" || providerGroup == "arabic2") && !rawSource.lowercase().contains("subdl")) {
                "SubDL"
            } else if (providerGroup.startsWith("subsource") && rawSource.lowercase().contains("عربي 2")) {
                "عربي 1"
            } else rawSource
    ...
    private fun isSubdlTrack(track: ExternalSubtitle): Boolean {
        val blob = listOf(track.source, track.label, track.release, track.url)
            .joinToString(" ")
            .lowercase()
        if (blob.contains("subsource") || blob.contains("عربي 1") || blob.contains("arabic 1")) return false
        return blob.contains("subdl") || blob.contains("عربي 2") || blob.contains("arabic2")
    }
    ...
    private fun pickAutoSubtitleTrack(
        ...
        val primaryArabic = tracks
            .filter { isArabicSubtitleTrack(it) && !isSubdlTrack(it) }
            .minByOrNull { subtitleAutoSelectionScore(it) }
        if (primaryArabic != null) {
            return if (exact != null && isArabicSubtitleTrack(exact) && !isSubdlTrack(exact)) exact else primaryArabic
        }
    ...
    private fun restoreVideoOnlyAfterPip() {
        ...
        try {
            ...
            playerView?.apply {
                ...
                controllerHideOnTouch = true
                controllerShowTimeoutMs = 4500
                resizeMode = effectivePlayerResizeMode(currentResizeMode)
                try { exoPlayer?.setVideoScalingMode(scalingModeForResizeMode(currentResizeMode)) } catch (_: Exception) {}
                alpha = 1f
                visibility = View.VISIBLE
                isClickable = true
                isEnabled = true
    ...
    private fun buildAndLoadMedia(
        ...
        val player = exoPlayer ?: run {
            ...
            exoPlayer = p
            playerView?.player = p
            playerView?.resizeMode = effectivePlayerResizeMode(currentResizeMode)
            p.repeatMode = Player.REPEAT_MODE_OFF
            p.setVideoScalingMode(scalingModeForResizeMode(currentResizeMode))
    ...
    private fun ensurePlayerUi() {
        ...
        fitButton = makeControlButton("FULL") { cycleResizeMode() }
    ...
    private fun resizeModeLabel(mode: Int = currentResizeMode): String {
        return when (mode) {
            AspectRatioFrameLayout.RESIZE_MODE_FILL -> "FULL"
            RESIZE_MODE_SCREEN_STRETCH -> "SCREEN"
            AspectRatioFrameLayout.RESIZE_MODE_ZOOM -> "CROP"
            AspectRatioFrameLayout.RESIZE_MODE_FIT -> "FIT"
            AspectRatioFrameLayout.RESIZE_MODE_FIXED_WIDTH -> "WIDTH"
            AspectRatioFrameLayout.RESIZE_MODE_FIXED_HEIGHT -> "HEIGHT"
            else -> "FULL"
        }
    }

    private fun effectivePlayerResizeMode(mode: Int = currentResizeMode): Int {
        return when (mode) {
            RESIZE_MODE_SCREEN_STRETCH -> AspectRatioFrameLayout.RESIZE_MODE_FILL
            else -> mode
        }
    }

    private fun scalingModeForResizeMode(mode: Int = currentResizeMode): Int {
        return when (mode) {
            RESIZE_MODE_SCREEN_STRETCH,
            AspectRatioFrameLayout.RESIZE_MODE_FIT,
            AspectRatioFrameLayout.RESIZE_MODE_FIXED_WIDTH,
            AspectRatioFrameLayout.RESIZE_MODE_FIXED_HEIGHT -> C.VIDEO_SCALING_MODE_SCALE_TO_FIT
            else -> C.VIDEO_SCALING_MODE_SCALE_TO_FIT_WITH_CROPPING
        }
    }

    private fun applyCurrentResizeMode() {
        playerContainer?.apply {
            if (currentResizeMode == RESIZE_MODE_SCREEN_STRETCH) {
                layoutParams = FrameLayout.LayoutParams(MATCH_PARENT, MATCH_PARENT).apply {
                    gravity = Gravity.CENTER
                    setMargins(0, 0, 0, 0)
                }
        ...
        try {
            exoPlayer?.setVideoScalingMode(scalingModeForResizeMode(currentResizeMode))
        } catch (_: Exception) {
        }
        fitButton?.text = resizeModeLabel(currentResizeMode)
    }

    private fun resetResizeModeToFull() {
        currentResizeMode = AspectRatioFrameLayout.RESIZE_MODE_FILL
        applyCurrentResizeMode()
    }
    ...
    private fun cycleResizeMode() {
        ...
        val currentIndex = order.indexOf(currentResizeMode).let { if (it >= 0) it else 0 }
        currentResizeMode = order[(currentIndex + 1) % order.size]
        applyCurrentResizeMode()
    }
    ...
    private fun showTextTracksDialog() {
        ...
        var selectedLang = tracks.firstOrNull { it.url == selectedSubtitleUrl }
            ...
            ?: when {
                langs.contains("Arabic") -> "Arabic"
                langs.isNotEmpty() -> langs.first()
                tracks.any { isSubdlTrack(it) } -> "__SUBDL__"
                else -> "Arabic"
            }
        ...
        val languageScroll = ScrollView(this).apply {
            isFillViewport = true
            overScrollMode = View.OVER_SCROLL_NEVER
            layoutParams = LinearLayout.LayoutParams(0, MATCH_PARENT, 0.20f)
        ...
        val optionsScroll = ScrollView(this).apply {
            isFillViewport = true
            overScrollMode = View.OVER_SCROLL_NEVER
            layoutParams = LinearLayout.LayoutParams(0, MATCH_PARENT, 0.55f)
        ...
        val settingsScroll = ScrollView(this).apply {
            isFillViewport = true
            overScrollMode = View.OVER_SCROLL_NEVER
            layoutParams = LinearLayout.LayoutParams(0, MATCH_PARENT, 0.25f)
        ...
        addSettingsStepper(
            ...
            onPlus = { subtitleVerticalOffsetPercent = (subtitleVerticalOffsetPercent + 1).coerceAtMost(35) },
        )
        ...
        fun renderOptions() {
            ...
            fun addRequestAgainButton() {
                if (selectedLang == "__SUBDL__") return
                optionsColumn.addView(TextView(this).apply {
                    text = "اعادة طلب"
                    gravity = Gravity.CENTER
                    setTextColor(Color.WHITE)
                    ...
                    setOnClickListener {
                        isEnabled = false
                        text = "جاري اعادة الطلب..."
                        try {
                            channel?.invokeMethod("onSubtitleRefreshRequested", mapOf("source" to "arabic1"))
                        } catch (_: Exception) {}
                    }
                })
            }

            if (selectedLang == "__SUBDL__") {
                addSourceSection("SubDL", subdlTracks)
            } else {
                addSourceSection("ترجمات عربي 1", otherTracks)
                addRequestAgainButton()
            }
        }
    ...
    private fun loadSubtitlePrefs() {
        ...
        if (savedVersion != CURRENT_SUB_STYLE_VERSION) {
            ...
            subtitleVerticalOffsetPercent = 10
        ...
        subtitleVerticalOffsetPercent = subtitlePrefs.getInt(PREF_SUB_OFFSET, 10).coerceIn(0, 35)
    ...
    private fun applySubtitleAppearance(view: androidx.media3.ui.SubtitleView? = playerView?.subtitleView) {
        ...
        subtitleView.setBottomPaddingFraction((subtitleVerticalOffsetPercent.coerceIn(0, 35)) / 100f)
        subtitleView.setPadding(dp(8), 0, dp(8), dp(10))
