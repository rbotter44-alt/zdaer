...
class ArabActivity : FlutterActivity() {

    override fun getDartEntrypointFunctionName(): String = StringVault.d("_GwncHudbFo")


    companion object {
        private val CHANNEL = StringVault.d("C5sitUl1gPZlhlkI")
        private const val MIN_PIP_RATIO = 0.41841
        private const val MAX_PIP_RATIO = 2.39
        private const val LOG_TAG = "NativePlayer"
        private var instance: ArabActivity? = null
...
instance = this
    ...
    override fun onWindowFocusChanged(hasFocus: Boolean) {
        ...
        if (hasFocus && nativePlayerActive) {
    ...
    private fun updateOverlayVisibility() {
        ...
        serverButton?.visibility = if (pageServerOptions.isNotEmpty()) View.VISIBLE else View.GONE
    ...
    private fun closeNativePlayer() {
        ...
        restoreNormalWindowAfterPlayer()
    ...
    private fun applyPlayerFullscreenWindow() {
        ...
        enterImmersiveMode(true)
        window.decorView.post { enterImmersiveMode(true) }
        playerContainer?.post { enterImmersiveMode(true) }
    }

    private fun restoreNormalWindowAfterPlayer() {
        @Suppress("DEPRECATION")
    ...
    private fun collectTrackChoices(tracks: Tracks, type: Int): List<TrackChoice> {
        ...
        for (group in tracks.groups) {
            ...
            for (i in 0 until group.length) {
                ...
                val label = when (type) {
                    C.TRACK_TYPE_VIDEO -> {
                        val resolution = if (format.height > 0) "${format.height}p" else "Video ${i + 1}"
                        val bitrate = if (format.bitrate > 0) " • ${format.bitrate / 1000} kbps" else ""
                        val codec = format.codecs?.takeIf { it.isNotBlank() }?.let { " • $it" } ?: ""
